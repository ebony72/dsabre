...
def main() -> None:
    ...
    # Isolation test: SAME legacy router/budgets/layout as the exact
    # reproduction, only enable_node_decay flipped off, to separate node_decay's
    # contribution to the submitted +23.4% from the monolithic layout's.
    hw = LegacyHardwareConfig(max_iterations=10000, deadlock_limit=50,
                              max_backup_attempts=50, enable_node_decay=False)
    hw_norelief = LegacyHardwareConfig(max_iterations=10000, deadlock_limit=50,
                                       max_backup_attempts=50, enable_node_decay=False,
                                       enable_congestion_relief=False)
    print(f"ISOLATION: legacy router/budgets/layout, enable_node_decay=False  "
