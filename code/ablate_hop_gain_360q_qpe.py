"""
ablate_hop_gain_360q_qpe.py — hop-gain ablation on 360-qubit QPEexact.

Runs dSABRE_BurstExt twice (hop_gain ON vs OFF) on the same 360q QPEexact
...
Output: code/results/ablate_hop_gain_360q_qpe.json
"""
...
QASM = os.path.expanduser(
    "~/Documents/telesabre/circuits/qasm_360/qpeexact_nativegates_ibm_qiskit_opt3_360.qasm"
)
...
def main():
    print(f"{'='*60}\n  hop-gain ablation: 360q QPEexact on H-grid 2x3 9x9 (486 phys)\n{'='*60}",
          flush=True)
    ...
    out = dict(
        meta=dict(
            ...
            circuit="qpeexact", qubits=qc.num_qubits, cx=n_cx,
    ...
    out_path = os.path.join(_RESULTS_DIR, "ablate_hop_gain_360q_qpe.json")
