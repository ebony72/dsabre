...
def _spread_ports(tile: nx.Graph, k: int) -> Tuple[int, ...]:
    """The k mutually most distant non-leaf qubits of a tile.

    Seeded with the farthest-apart pair `_heavy_hex_ports` returns and grown by
    farthest-point insertion, so k=2 reproduces that pair exactly and a core
    needing more ports (a star hub takes one per spoke) still spreads them.
    """
    dist = dict(nx.all_pairs_shortest_path_length(tile))
    cands = sorted(n for n in tile if tile.degree(n) >= 2)
    chosen = list(_heavy_hex_ports(tile))[:max(k, 0)]
    while len(chosen) < k:
        chosen.append(max((n for n in cands if n not in chosen),
                          key=lambda n: (min(dist[n][c] for c in chosen), -n)))
    return tuple(chosen)


def build_heavy_hex_architecture(num_cores: int = 4,
                                 core_graph: str = "ring"
                                 ) -> DistributedArchitecture:
    """IBM 27-qubit heavy-hex cores on a ring or star core graph.

    Each core is a Falcon-family heavy-hex tile rather than a 2-D grid
    (4 cores = 108 physical qubits).  ``core_graph`` selects how the cores are
    wired together, the second axis of the architecture-independence check:

    ``ring``  a cycle with one inter-core link per adjacent pair (4 links at
              num_cores=4, matching the B-grid's link budget).  Every core has
              degree 2; core c reaches c-1 through its low port and c+1 through
              its high port.
    ``star``  core 0 is a hub holding one port per spoke, every other core a
              degree-1 leaf on its low port (3 links at num_cores=4).  All
              inter-core traffic funnels through the hub, so the core graph is
              irregular and its centre is a capacity bottleneck.
    """
    qpc = len(set(sum(map(list, IBM_HEAVY_HEX_27), [])))  # 27
    tile = nx.Graph(IBM_HEAVY_HEX_27)
    ...
    if core_graph == "ring":
        lo, hi = _heavy_hex_ports(tile)
        inter_links = [(c * qpc + hi, ((c + 1) % num_cores) * qpc + lo)
                       for c in range(num_cores)]
        if num_cores == 2:                  # a 2-cycle would duplicate the link
            inter_links = inter_links[:1]
    elif core_graph == "star":
        hub = _spread_ports(tile, num_cores - 1)
        leaf = _heavy_hex_ports(tile)[0]
        inter_links = [(0 * qpc + hub[i], (i + 1) * qpc + leaf)
                       for i in range(num_cores - 1)]
    else:
        raise ValueError(f"unknown core_graph {core_graph!r}; "
                         f"expected 'ring' or 'star'")
