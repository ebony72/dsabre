...
Proposition~3 (progress and termination, with a bound of one core-graph
diameter of EPR pairs per forced escape).  Each is attached to an
experimental result it explains rather than standing alone.

\item \textbf{One scoring term removed.}  The hop-gain reward
$g_\mathrm{hop}$ is retired.  Ablation showed it inert, and
Proposition~1 explains why: it re-counts a directional signal that
$\Delta_F$ already carries.  The score is now four terms with one fewer
hyperparameter, and all reported numbers are regenerated without it.
...
64-qubit suite on a ring of four IBM 27-qubit heavy-hex cores, with no
router or layout changes, addressing the tension between the abstract
architecture model and the grid-only evaluation.

\item \textbf{Comparison methodology.}  A new table states what one EPR
pair buys under each compiler's cost model, and the differing
communication primitives are addressed by a layout-controlled
comparison and an entanglement-lifetime sweep.
...
implicit.
\where{Section~IV-A ``What one EPR pair buys'' and Table~V;
lifetime sweep in Section~IV-B; Section~VI}
...
reports the 64-qubit suite on a ring of four IBM 27-qubit heavy-hex
...
automatically.  \TeleSABRE{} is run on the same architecture.
\where{new Section~IV-D, Table~IX}
...
\response
Addressed by the same restructure, plus one concrete consequence: the
decomposition was not minimal, and we have reduced it.  The hop-gain
term is removed, because Proposition~1 shows it re-counts a signal
$\Delta_F$ already carries on a graph whose inter-core links are
weighted $w_\mathrm{link} \gg 1$, and the ablation confirms every
circuit in two suites ties exactly without it.  The remaining four
terms map one-to-one onto the three groups of the design principle.
\where{Section~III-C, Eq.~(6) and following; Table~III;
Section~IV-E}
...
pair may serve.
\where{Section~IV-A, Table~V; layout-controlled results and
...
size.  Proposition~3 gives progress and termination guarantees, with an
EPR bound of one core-graph diameter per forced escape, and explains
why \dSABRE{} completes circuits on which a fixed safety-valve cap
fails.
\where{Propositions 1--3 in Sections~III-A, III-D, III-F}
...
report both regimes.
\where{Section~IV-B, layout-controlled paragraph and
Table~VIII}
...
\where{Abstract; Contributions bullets~1 and~3; Section~IV-E and
Table~XI}
...
leaving it implicit.
\where{Tables~VI--VII; Section~IV-B; new paragraph in Section~VI}
...
\where{Figure~4 caption; Table~IV}
