...
$M-\deg(c)$.  Section~IV-C re-runs it with both corrected.  The data
correction \emph{helps} \texttt{pytket-dqc} and we adopt it: its 25-qubit
geometric mean improves from $35.3$ to $27.5$ and the 36-qubit from
$12.3$ to $10.6$, so \dSABRE{}'s margin narrows to $-44.3\%$ at 25
qubits and the 36-qubit comparison crosses to parity.  We report the
narrower figures.  Separately, the communication bound is not enforced
by the distributors at all---setting it leaves every reported cost
unchanged, because the check happens only at circuit materialisation---
and on that test four of six 25-qubit distributions turn out not to be
executable on the device, needing $7$ to $19$ link qubits per core
