...
\appendices
% This file is compiled as its own document, yet cites the main paper's
% equations and tables through xr.  Prefix the appendix's own numbers so
% "Eq. (1)" and "Table IV" cannot mean two different things at once.
...
four sites, and checkpoint--rollback is enabled.  An ordinary teleport is
...
initial layout obeys Eq.~\eqref{eq:safe_invariant}, which
Section~\ref{sec:initial_mapping} enforces by reserving sites per core.  That
last one is not cosmetic: the global condition Eq.~\eqref{eq:safe_feasible}
...
\paragraph*{What the capacity ablation switches off}
The two rows of Table~\ref{tab:mech} that drop the floor---\emph{no legality
floor} and \emph{neither floor nor score}---run the router in score-only mode;
\emph{strict floor} raises it to $\phi_1=3$ with safe mode still on.
Checkpoint--rollback is unaffected---both designs carry it, and neither takes
it---but the guaranteed transaction is not reached, and an ordinary teleport
reverts to requiring one free slot in its landing core before the move instead
of $\phi_1=2$.  A landing core may therefore end full---exactly what
Lemma~\ref{lem:nofull} rules out under the shipped rule---so an occupied port
there can no longer be evacuated, the greedy backup those arms fall back on can
fail, and that is how the \emph{neither} arm loses two circuits.  The relays that
backup issues still ask for two free slots at each receiving core, so
Eq.~\eqref{eq:teleportge2} is respected by every teleport a relay emits even
in the ablated arms; it is ordinary routing, not recovery, that the ablation
frees.
...
% -----------------------------------------------------------------------------
\section{Complexity: Derivation and Validation}
\label{app:complexity}

Section~III-H states the bound of Eq.~\eqref{eq:complexity} and the reasons it is
loose.  This appendix derives every term of it, records the four structural
properties the derivation relies on but does not argue, and reports the
measurements that check the derivation against the implementation which
produced every result in the paper.

\subsection{Assumptions}
\label{app:complexity_assumptions}

Throughout, $N$ is the number of gates, $n$ the number of logical qubits, $K$
the number of cores, $M$ the largest number of physical sites in one core, and
$D_K$ and $D_M$ the core-graph diameter and the largest intra-core diameter.
Four quantities are treated as constants: the extended-set limit $L$, the
intra-core degree, the core-graph degree, and the number of links joining one
adjacent pair of cores.  A core therefore has $O(M)$ edges, the core graph has
$O(K)$, and the device has $O(K)$ communication ports; the last two are what
bound the teleport candidate count at $O(n)$ below.

Four structural properties are used without being re-argued here: (1)~the
circuit DAG has bounded degree; (2)~the front layer is a matching, so a
candidate SWAP changes at most two front-layer distance terms; (3)~BFS layers
of the extended set are matchings, which is what bounds the per-update cost of
building $E$ at $O(n+L)$ after a gate retirement and $O(L)$ for its per-core
restrictions; and (4)~gates retire only from the front layer, so remaining
in-degrees and rollback logs can be maintained incrementally rather than
rebuilt.  Property~(4) is the one that would fail silently if it were
violated, which is why it is measured in
Table~\ref{tab:appendix-complexity-checks} alongside~(2).

\subsection{Precomputation}
\label{app:complexity_pre}

All-pairs breadth-first search inside every core costs $O(KM^2)$ for
$\dintra$, and $O(KM^2D_M)$ to materialise the intra-core paths, whose $KM^2$
entries have $O(D_M)$ sites each; the same search on the core graph costs
$O(K^2)$ for $\dcore$ and $O(K^2D_K)$ for the core paths.  Dijkstra over the
port graph gives the port-level table $\dport$ in $O(K^2\log K)$, and $\dphys$
is then composed on demand by Eq.~\eqref{eq:dphys} rather than materialised,
which avoids the $\Theta(P^2)$ table a whole-chip all-pairs search would build.
The precomputation is therefore
\begin{equation}
T_{\mathrm{pre}}=O\!\left(KM^2D_M+K^2(D_K+\log K)\right),
\label{eq:app_precompute}
\end{equation}
in $O(KM^2D_M+K^2D_K)$ space, which the materialised paths dominate.  Storing
next-hop or predecessor pointers instead of path sequences would drop the
$D_M$ and $D_K$ factors from both time and space, leaving
$O(KM^2+K^2\log K)$ time and $O(KM^2+K^2)$ space, and charging $O(D_M)$ or
$O(D_K)$ only for each path actually reconstructed.  Precomputation is paid
once per architecture, not once per circuit, and is excluded from the bounds
below and from the compile times reported in Section~IV-C.

\subsection{One routing iteration}
\label{app:complexity_iter}

The extended set is rebuilt only after a gate retires, at the $O(L)$ per
update of Section~III-E, and hence $O(NL)=O(N)$ over a route.

In one local-routing iteration, each active core enumerates $O(M)$ candidate
edges---its front gates are qubit-disjoint by property~(2), so
$|F_c|\le M/2$---and scores each in $O(L)$, since a SWAP moves at most two
front-layer operands.  Over all active cores the iteration costs $O(n)$,
because $\sum_c|F_c|\le n/2$.

One teleport-selection iteration generates $O(n)$ candidates: front-layer
gates are qubit-disjoint, so there are $O(n)$ inter-core source operands, and
bounded core degree and link multiplicity leave $O(1)$ adjacent-link choices
per operand.  If free-slot counts and locations are recomputed from the
inverse layout, scanning all $K$ cores costs $O(KM)$.  In the current
implementation each candidate may additionally scan $O(M)$ sites in its
landing core to identify an eviction target or a free site, giving $O(nM)$
over all candidates.  Hence
\begin{equation}
T_{\mathrm{tele}} = O(KM+nM) = O\bigl(M(K+n)\bigr),
\label{eq:app_tele}
\end{equation}
which dominates the $O(n)$ local-routing cost.  Both scans are implementation
artefacts: per-core free-slot counters and free-site sets maintained
incrementally would reduce or remove them, leaving $O(n)$ per iteration.

\subsection{Ordinary iterations and recovery}
\label{app:complexity_ord}

Recovery is invoked after at most $L_{\mathrm{deadlock}}=4D_K(D_M+1)$ stalled
iterations and, by Theorem~\ref{thm:completion}, retires at least one gate.
There are at most $N$ intervals between successive gate retirements, so the
number of ordinary iterations is $O(ND_K(D_M+1))$ and
\begin{equation}
T_{\mathrm{ord}}=O\!\left(ND_K(D_M+1)\,M(K+n)\right).
\label{eq:app_tord}
\end{equation}

At the beginning of recovery, Lemma~\ref{lem:nofull} gives $f_c\ge1$ for every
core, so restoring the two-site reserve relays at most one vacancy to each of
at most $K$ deficient cores.  For each relay the current implementation spends
$O(KM)$ searching for a donor and at most $O(MD_K)$ moving the vacancy over a
core path of length at most $D_K$, with an $O(M)$ scan at each hop.  Reserve
restoration therefore costs
\[
  O\!\left(K(KM+MD_K)\right) = O\!\left(KM(K+D_K)\right).
\]
Moving the blocked gate's operand to the meeting core costs $O(MD_K)$ more and
the final intra-core SWAPs $O(D_M)\subseteq O(M)$, both absorbed by that
bound.  Since each transaction retires at least one gate, there are at most
$N$ of them, giving
\begin{equation}
T_{\mathrm{rec}} = O\!\left(NKM(K+D_K)\right).
\label{eq:app_trec}
\end{equation}
This is conservative for the scan-based implementation; maintained donor and
free-site structures would reduce the search factors, as would recording that
recovery fires far less than once per gate
(Table~\ref{tab:tier2} of the main paper).

\subsection{Common bound}
\label{app:complexity_total}

Summing extended-set maintenance, Eq.~\eqref{eq:app_tord}, and
Eq.~\eqref{eq:app_trec}, and using $D_M+1=O(D_M)$, $K+n=O(n)$ and
$K+D_K=O(K)$---valid since $D_M\ge1$, $K=O(n)$ and $D_K\le K-1$---gives the
common bound of the main paper,
\[
  T_{\mathrm{total}} = O\!\left(NM\left(D_KD_Mn+K^2\right)\right).
\]
The $K^2$ term is the recovery contribution and the $M$ factor the candidate
scans, so the two sources of looseness are separable: with maintained
free-slot structures and recovery charged at its measured rate rather than
once per gate, the same derivation gives $O(ND_KD_Mn)$.  For a core graph and
cores that are bounded-aspect-ratio two-dimensional grids,
$D_K=O(\sqrt{K})$ and $D_M=O(\sqrt{M})$, so $D_KD_M=O(\sqrt{P})$ with
$P=KM$ and
\[
  T_{\mathrm{total}} = O\!\left(NM\left(\sqrt{P}\,n+K^2\right)\right).
\]

\subsection{Validation}
\label{app:complexity_valid}
...
$5{\times}5$ cores), which Eq.~\eqref{eq:complexity} charges through $K^2$.  An
...
\textbf{Non-convergence criterion.}  A \TeleSABRE{} failure reported in the
main text is the tool's own verdict: after a fixed number of iterations
without progress its safety valve restores the last layout that made
progress and records a deadlock, and if the stall persists it ends the
attempt.  Every seed exhausts its ten internal attempts this way, well
