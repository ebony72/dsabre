...
order.  Across \rev{21} MQT-Bench circuits
at 25, 36, and 64 logical qubits, \dSABRE{} reduces geometric-mean
EPR consumption by \rev{$41$--$46\%$} over \TeleSABRE{}.  \rev{Against the
...
\rev{The asymmetry is also \emph{temporal}: a gate-teleportation e-bit
must stay entangled for the full duration of the hyperedge it serves,
whereas a teledata e-bit is consumed instantaneously by its teleport.
Section~\ref{sec:main_results} quantifies what this unbounded-lifetime
assumption is worth (Table~\ref{tab:dmax}).}
...
$D_{\max}{=}1$).  Three structural observations.  (i)~The circuits on
which gate teleportation wins at $D_{\max}{=}\infty$ are exactly those
whose distributions hold links across many gates, and they inflate
$2.7$--$4.8\times$ at $D_{\max}{=}3$ (AE $137\!\to\!423$, QFT
$190\!\to\!572$, QNN $540\!\to\!2435$, QAOA $472\!\to\!1282$); the
sharpest case is QPEexact, \texttt{pytket-dqc}'s largest unbounded win
($+182.3\%$ in Table~\ref{tab:main64}), which inflates $79\!\to\!378$
and flips to a \dSABRE{} advantage ($-28.3\%$).  QNN's winning
distribution
requires individual link qubits to serve up to $102$ gates while
remaining entangled across $4262$ of the circuit's $8126$ CX
time-slots---more than half its execution.  (ii)~GHZ and Graphstate
are $D_{\max}$-\emph{insensitive}: their hyperedges hold roughly one
gate each, so these residual wins reflect partition quality rather
than held entanglement, and the shared-mapping experiment above shows
\dSABRE{} matches them when routed from the same partition.
(iii)~Dense circuits move least (Random $908\!\to\!967$, Multiplier
$2950\!\to\!5004$): with no or short
shared-control runs there is little amortisation to lose, and
\dSABRE{} already led those rows.  The same sweep on the smaller
suites only widens \dSABRE{}'s margins: at $D_{\max}{=}3$ the best
\texttt{pytket-dqc} pipeline inflates AE-25 from $48$ to $136$ e-bits
and QFT-25 from $59$ to $164$, and QPEexact-36 inflates from $44$ to
$192$, turning the one structured 36q circuit it won
(Table~\ref{tab:main36}, $+32.7\%$) into a $66\%$ \dSABRE{}
advantage.  A $D_{\max}$-aware re-partition could soften, though not
eliminate, this inflation; \texttt{pytket-dqc} currently offers no
such option.  We conclude that gate-teleportation's e-bit advantage on
structured circuits is conditional on hardware sustaining
entanglement across ${\gtrsim}5$--$10$ gates per EPR pair, whereas
teledata routing delivers its counts under any lifetime.}
