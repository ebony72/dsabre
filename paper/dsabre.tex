...
EPR consumption by $41$--$53\%$ over \TeleSABRE{}.  Against the
...
\dSABRE{} uses a \texttt{SabreLayout}-based initial layout with per-core
...
a SWAP as the front-layer distance gain $\Delta_F$ plus a
decay-weighted lookahead ${\Delta}_E$, both in $\Delta$-form
(old minus new distance) on gates sharing a qubit with the move.
...
teleportation objective.  On the per-core adaptive layout its
...
$41$--$53\%$ over \TeleSABRE{}~\cite{russo2025telesabre}.  It also
...
uses a \texttt{SabreLayout}-based initial layout with per-core
...
\begin{figure}[t]
  ...
  \caption{Running example for inter-core teleportation scoring (2$\times$3
    ...
    combining the fewest staging SWAPs with positive $\Delta_F$ and hop
...
A wins via cheap staging combined with positive $\Delta_F$ and hop gain;
B loses because $q_1$ moves away from $C_5$ ($\Delta_F$ negative);
...
The per-core adaptive corner reservation
(Section~\ref{sec:layout}) supplies every core with escape slots, which
...
Against pytket-dqc's strongest distributor the comparison
...
Congestion relief is near-neutral on both suites once the per-core
...
landing core.  We exclude the four lowest-degree \emph{corner}
physical qubits from the coupling map passed to SabreLayout---in
both B-grid and H-grid architectures they are furthest from any
inter-core link---which guarantees four free slots per affected core
to serve as eviction scratch space and to keep the core below the
capacity threshold.
...
At \emph{100q}, \dSABRE{} now edges \TeleSABRE{} ($-10.1\%$ EPR under the
...
pending regeneration on the per-core layout (Section~\ref{sec:layout}).
