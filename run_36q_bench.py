"""
3-way benchmark on 36-qubit circuits, B_grid_2_2_4_4 device
(4 cores of 4×4 qubits = 64 physical slots).

Circuits: BV, DJ, QAOA, QPEexact, VQE-SU2, W-state  (all 36q, IBM native gates)

Columns reported:
  TS      — TeleSABRE best (teledata + 2×telegate), 2 seeds
  dS      — dSABRE best across layouts A/B/C, 3 trials each
  Bu      — BurstDSABRE best across layouts A/B/C, 3 trials each
  Bu†     — BurstDSABRE with TeleSABRE's initial layout
  dS†     — dSABRE with TeleSABRE's initial layout
  Bu/dS%  — Bu_best vs dS_best reduction %
  Bu†/TS% — Bu† vs TS_EPR reduction %
"""
...
hw       = HardwareConfig()
hw_large = HardwareConfig(deadlock_limit=100, max_backup_attempts=100,
...
def main():
    ...
    for qf in qasm_files:
        ...
        # Strip measurements/barriers — router handles unitary gates only
